            ...
            if (forwardObj && forwardObj.forward && (forwardObj.forward.url || forwardObj.forward.local)) {
                ...
                var responseCB = function responseCB() {
                    if (forwardObj.forward.local) {
                        console.log(logColor.FgGreen + '%s' + logColor.Reset, 'URL: ' + forwardObj.url + ' 被代理到本地 ' + forwardObj.forward.local + '，设置延迟：' + (forwardObj.forward.timeout ? forwardObj.forward.timeout : 0) + '毫秒');
                        res.writeHead('200');
                        ...
                        console.log(logColor.FgGreen + '%s' + logColor.Reset, 'URL: ' + forwardObj.url + ' 被转发到 ' + forwardObj.forward.url + '，设置延迟：' + (forwardObj.forward.timeout ? forwardObj.forward.timeout : 0) + '毫秒');
